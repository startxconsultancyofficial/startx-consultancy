...
<template>
  <div class="min-h-screen flex flex-col">
    ...
    <button 
      ...
      :class="handStore.isActive ? 'bg-amber-500/20 border-amber-500/50 text-amber-500 hover:bg-amber-500/30' : 'bg-gray-900/50 border-gray-700 text-white/50 hover:bg-gray-800 focus:outline-none'"
    ...
    >
      ...
      ></div>
      <Hand class="w-6 h-6" :class="{ 'animate-pulse': handStore.isActive }" />
      ...
      >
        ...
        <ul class="space-y-3 text-xs">
          <li class="flex items-center gap-3">
            ...
            <span><strong class="text-white">Fist:</strong> Gather particles</span>
          </li>
          <li class="flex items-center gap-3">
            <span class="text-xl">✌️</span> 
            <span><strong class="text-white">2 Fingers:</strong> Scroll page</span>
          </li>
          <li class="flex items-center gap-3">
            <span class="text-xl">☝️</span> 
            <span><strong class="text-white">1 Finger:</strong> Move cursor</span>
          </li>
          <li class="flex items-center gap-3">
            <span class="text-xl">🤏</span> 
            <span><strong class="text-white">Pinch:</strong> Click screen</span>
          </li>
        </ul>
    ...
    <div
      v-if="handStore.isActive && handStore.isHandPresent"
      class="fixed z-[9999] pointer-events-none rounded-full border-2 transition-all duration-75"
      :class="[
        handStore.isPinching ? 'w-4 h-4 bg-amber-500 border-amber-300' : 'w-6 h-6 bg-white/30 border-white/80',
        { 'opacity-0': handStore.gesture !== 'Pointing_Up' && !handStore.isPinching }
      ]"
      :style="{
